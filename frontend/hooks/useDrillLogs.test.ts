...
import { useDrillLogs } from './useDrillLogs';
...
describe('useDrillLogs', () => {
  ...
  it('should set loading to true when fetching begins', async () => {
    ...
    const { result, waitForNextUpdate } = renderHook(() => useDrillLogs(playerId));
    expect(result.current.loading).toBe(true);
    await waitForNextUpdate();
  });

  it('should populate drills and set loading to false on successful fetch', async () => {
    const mockDrills = [{ id: '1', playerId: 'test-player-id' }];
    (getDocs as jest.Mock).mockResolvedValueOnce({ docs: mockDrills.map(doc => ({ id: doc.id, data: () => doc })) });
    const { result, waitForNextUpdate } = renderHook(() => useDrillLogs(playerId));
    await waitForNextUpdate();
  ...
  it('should update state when playerId changes', async () => {
    const { result, rerender, waitForNextUpdate } = renderHook(({ id }) => useDrillLogs(id), {
