  ...
  const getRefereeAvailability = useCallback(async (date: string, timeSlot: string) => {
    try {
      ...
      return MOCK_DATA.referees.filter(ref => {
        ...
        return (ref.availability as any)[dayOfWeek]?.[timeOfDay] && ref.currentAssignments.length < ref.maxGamesPerWeek;
      });
    } catch (err) {
  ...
  const getMissingCertifications = useCallback(async () => {
    try {
      ...
      return MOCK_DATA.coaches.filter(coach => 
        coach.certifications.some(cert => cert.status === 'failed' as any) ||
        coach.backgroundCheck.status === 'failed' as any
      );
    } catch (err) {
