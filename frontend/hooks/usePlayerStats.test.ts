...
describe('usePlayerStats', () => {
  ...
  it('should set loading to true when fetching begins', async () => {
    ...
    const { result, waitForNextUpdate } = renderHook(() => usePlayerStats(playerId));
    expect(result.current.loading).toBe(true);
    await waitForNextUpdate();
  });

  it('should populate stats and set loading to false on successful fetch', async () => {
    const mockStats = [{ id: '1', playerId: 'test-player-id' }];
    (getDocs as jest.Mock).mockResolvedValueOnce({ docs: mockStats.map(doc => ({ id: doc.id, data: () => doc })) });
    const { result, waitForNextUpdate } = renderHook(() => usePlayerStats(playerId));
    await waitForNextUpdate();
  ...
  it('should update state when playerId changes', async () => {
    const { result, rerender, waitForNextUpdate } = renderHook(({ id }) => usePlayerStats(id), {
