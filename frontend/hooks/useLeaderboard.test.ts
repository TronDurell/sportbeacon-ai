...
describe('useLeaderboard', () => {
  ...
  it('should set loading to true when fetching begins', async () => {
    ...
    const { result, waitForNextUpdate } = renderHook(() => useLeaderboard(playerId));
    expect(result.current.loading).toBe(true);
    await waitForNextUpdate();
  });

  it('should populate leaderboard and set loading to false on successful fetch', async () => {
    const mockLeaderboard = [{ id: '1', playerId: 'test-player-id' }];
    (getDocs as jest.Mock).mockResolvedValueOnce({ docs: mockLeaderboard.map(doc => ({ id: doc.id, data: () => doc })) });
    const { result, waitForNextUpdate } = renderHook(() => useLeaderboard(playerId));
    await waitForNextUpdate();
  ...
  it('should update state when playerId changes', async () => {
    const { result, rerender, waitForNextUpdate } = renderHook(({ id }) => useLeaderboard(id), {
