import { collection, doc, where, orderBy, limit, Timestamp } from "firebase/firestore";
 ...
 */
export const buildLocationsQuery = (filters = {}) => {
    const constraints = [];
    if (filters.sport) {
 ...
 */
export const buildLocationPostsQuery = (locationId, filters = {}, pageSize = 20) => {
    const constraints = [
 ...
 */
export const buildFollowedLocationsQuery = (userId) => {
    return [
        where("userId", "==", userId),
 ...
 */
export const buildHomeLocationFeedQuery = (userId, pageSize = 20) => {
    return [
        orderBy("createdAt", "desc"),
        limit(pageSize)
    ];
