...
export const useDrillScrollSessionManager = () => {
    ...
    const [currentDrillSession, setCurrentDrillSession] = useState(null);
    ...
    const takeDrillAction = useCallback((action) => {
        if (!currentDrillSession)
            return;
    ...
    const getDrillSessionCTA = useCallback(() => {
        if (!currentDrillSession || !sessionCompleted)
            return null;
        return {
