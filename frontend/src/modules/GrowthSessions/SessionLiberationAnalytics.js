...
export class SessionLiberationAnalytics {
    ...
    startSession(userId, role, intent, sessionType) {
    ...
    logScrollEvent(scrollY, scrollDirection, scrollSpeed, elementId, pageSection) {
    ...
    logIntervention(type, trigger, aiPrompt, effectiveness) {
        if (!this.currentSession)
            return "";
    ...
    logInterventionResponse(interventionId, response, userFeedback) {
        if (!this.currentSession)
            return;
    ...
    logAction(type, description, aiPrompt, relatedContent, location) {
        if (!this.currentSession)
            return "";
    ...
    completeAction(actionId, success, impact) {
        if (!this.currentSession)
            return;
    ...
    calculateTotalScrollTime(session) {
    ...
    calculateScrollEfficiency(session) {
        if (session.scrollEvents.length === 0)
            return 0;
        const meaningfulScrolls = session.scrollEvents.filter(event => event.scrollSpeed > 50 && event.timeSinceLastScroll > 100).length;
        return meaningfulScrolls / session.scrollEvents.length;
    }
    calculateEngagementDepth(session) {
        if (session.scrollEvents.length === 0)
            return 0;
    ...
    calculateBounceRate(session) {
        if (session.scrollEvents.length === 0)
            return 1;
        const quickExits = session.scrollEvents.filter(event => event.timeSinceLastScroll < 5000 // Less than 5 seconds
        ).length;
        return quickExits / session.scrollEvents.length;
    }
    calculateInterventionEffectiveness(session) {
        if (session.interventions.length === 0)
            return 0;
        const totalEffectiveness = session.interventions.reduce((sum, intervention) => sum + intervention.effectiveness, 0);
        return totalEffectiveness / session.interventions.length;
    }
    calculateActionCompletionRate(session) {
        if (session.actionsTaken.length === 0)
            return 0;
        const completedActions = session.actionsTaken.filter(action => action.completionTime !== undefined).length;
        return completedActions / session.actionsTaken.length;
    }
    calculateConversionRate(session) {
        if (session.scrollEvents.length === 0)
            return 0;
    ...
    calculateSessionQuality(session) {
        ...
        return (metrics.scrollEfficiency * weights.scrollEfficiency +
    ...
    updateTrendingInsights(session) {
        const metrics = session.metrics;
...
export const useSessionLiberationAnalytics = () => {
    ...
    const startAnalyticsSession = useCallback((sessionType) => {
        if (!user)
            return null;
        return analytics.startSession(user.id, user.role, userIntent || "explore", sessionType);
    }, [user, userIntent, analytics]);
    const logScroll = useCallback((scrollY, direction, speed) => {
        analytics.logScrollEvent(scrollY, direction, speed);
    }, [analytics]);
    const logIntervention = useCallback((type, trigger) => {
        return analytics.logIntervention(type, trigger);
    }, [analytics]);
    const logInterventionResponse = useCallback((interventionId, response) => {
        analytics.logInterventionResponse(interventionId, response);
    }, [analytics]);
    const logAction = useCallback((type, description, aiPrompt) => {
