...
export const usePlaymakerIntentEngine = () => {
    ...
    const [playmakerSession, setPlaymakerSession] = useState(null);
    ...
    const rapidScrollTimerRef = useRef(null);
    ...
    const updatePlaymakerSession = useCallback((updates) => {
        if (!playmakerSession)
            return;
    ...
    const checkForCoachNudge = useCallback(() => {
        if (!autopilot || !user || !playmakerSession)
            return;
        const now = Date.now();
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (rapidScrollTimerRef.current)
                clearInterval(rapidScrollTimerRef.current);
