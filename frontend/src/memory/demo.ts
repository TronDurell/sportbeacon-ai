...
export interface MemoryDemoOptions {
  tenantId: string;
  enabled?: boolean;
}

export class MemoryDemo {
  private sdk: MemorySDK | null = null;
  private tenantId: string;
  private enabled: boolean;

  constructor(options: MemoryDemoOptions) {
  ...
  async demonstrateUsage(): Promise<void> {
    ...
    try {
      ...
      if (trainingMemories.length > 0) {
        ...
        const newScore = await this.sdk.learn(
          trainingMemories[0].id!,
          'user',
          this.sdk['uid']!,
          feedback
        );
...
export function useMemoryDemo(tenantId: string) {
  const [demo, setDemo] = useState<MemoryDemo | null>(null);
