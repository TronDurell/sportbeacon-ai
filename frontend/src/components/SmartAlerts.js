...
import { Target, TrendingUp, Clock, Users, Zap, X, Star, Trophy, Lightbulb, Heart } from "lucide-react";
const SmartAlerts = ({ className = "", position = "top-right", maxAlerts = 3, roleBased = true, intentAware = true }) => {
    const { user } = useAuth();
    ...
    const generateRoleBasedAlerts = (role) => {
        const alerts = [];
        switch (role) {
            case "player":
                alerts.push({
    ...
    const generateIntentBasedAlerts = (intent) => {
        const alerts = [];
        switch (intent) {
            case "train":
                alerts.push({
    ...
    const generateSessionBasedAlerts = () => {
        const alerts = [];
    ...
    const generateAchievementAlerts = async () => {
        const alerts = [];
    ...
    const generateMotivationAlerts = () => {
        const alerts = [];
        const motivations = [
            {
                id: "motivation_1",
    ...
    const filterAndPrioritizeAlerts = (alerts) => {
        const now = Date.now();
        return alerts
            .filter(alert => {
            ...
            if (alert.role && alert.role !== user?.role)
                return false;
            // Filter by intent if specified
            if (alert.intent && alert.intent !== userIntent)
                return false;
            return true;
        })
            .sort((a, b) => {
    ...
    const handleAction = useCallback(async (actionType, aiPrompt) => {
        try {
    ...
    return (_jsx("div", { className: `fixed z-50 ${getPositionStyles()} space-y-3 ${className}`, children: _jsx(AnimatePresence, { children: alerts.map((alert) => (_jsx(motion.div, { initial: { opacity: 0, x: position.includes("right") ? 100 : -100, scale: 0.9 }, animate: { opacity: 1, x: 0, scale: 1 }, exit: { opacity: 0, x: position.includes("right") ? 100 : -100, scale: 0.9 }, transition: { duration: 0.3, ease: "easeOut" }, className: getAlertStyles(alert.type), children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex items-start gap-3", children: [alert.icon && (_jsx("div", { className: "flex-shrink-0 mt-0.5", children: alert.icon })), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h4", { className: "font-semibold text-sm mb-1", children: alert.title }), _jsx("p", { className: "text-sm opacity-90", children: alert.message }), alert.actions && alert.actions.length > 0 && (_jsx("div", { className: "flex gap-2 mt-3", children: alert.actions.map((action, index) => (_jsx("button", { onClick: action.onClick, className: `px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${action.variant === "primary"
                                                    ? "bg-blue-600 text-white hover:bg-blue-700"
                                                    : action.variant === "secondary"
                                                        ? "bg-gray-200 text-gray-800 hover:bg-gray-300"
                                                        : action.variant === "ghost"
                                                            ? "bg-transparent text-gray-600 hover:bg-gray-100"
                                                            : "bg-red-600 text-white hover:bg-red-700"}`, children: action.label }, index))) }))] })] }), alert.dismissible && (_jsx("button", { onClick: () => dismissAlert(alert.id), className: "flex-shrink-0 ml-2 text-gray-400 hover:text-gray-600 transition-colors", children: _jsx(X, { className: "w-4 h-4" }) }))] }) }, alert.id))) }) }));
};
