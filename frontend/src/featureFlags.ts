...
export interface FeatureFlags {
  ...
  AGENT_ANALYTICS_ENABLED: boolean;
}
 ...
 */
export function getFeatureFlagStatus(): Record<string, { enabled: boolean; source: 'env' | 'localStorage' | 'default' }> {
  const envFlags = {
    AGENTS_ENABLED: process.env.REACT_APP_AGENTS_ENABLED === 'true',
    MCP_ENABLED: process.env.REACT_APP_MCP_ENABLED === 'true',
    ASSISTANT_ENABLED: process.env.REACT_APP_ASSISTANT_ENABLED === 'true',
  ...
  const result: Record<string, { enabled: boolean; source: 'env' | 'localStorage' | 'default' }> = {};
