...
import { doc, collection, query, where, orderBy, limit, startAfter, onSnapshot, addDoc, deleteDoc, updateDoc, setDoc, getDocs, getDoc, serverTimestamp, Timestamp } from "firebase/firestore";
 ...
 */
export function useLocationPosts(locationId, filters = {}, pageSize = 20) {
    ...
    const loadPosts = useCallback(async (isInitial = false) => {
        ...
        try {
            setLoading(true);
            setError(null);
            let q = query(collection(db, "locations", locationId, "threads"), orderBy("pinned", "desc"), orderBy("createdAt", "desc"), limit(pageSize));
            // Apply filters
            if (filters.type) {
 ...
 */
export function useIsFollowingLocation(locationId, userId) {
 ...
 */
export function useFollowedLocations(userId) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        const q = query(collection(db, "follows_locations"), where("userId", "==", userId), orderBy("createdAt", "desc"));
 ...
 */
export function useHomeLocationFeed(userId, pageSize = 20) {
    ...
    const loadFeed = useCallback(async (isInitial = false) => {
        ...
        try {
            setLoading(true);
            setError(null);
            let q = query(collection(db, "users", userId, "home_location_feed"), orderBy("createdAt", "desc"), limit(pageSize));
            if (!isInitial && lastDoc) {
                q = query(q, startAfter(lastDoc));
            }
            const snapshot = await getDocs(q);
 ...
 */
export async function followLocation(locationId, userId, notifications = "all") {
    try {
        const followId = `${userId}_${locationId}`;
        const followRef = doc(db, "follows_locations", followId);
 ...
 * @returns Success status and error message
 */
export async function unfollowLocation(locationId, userId) {
    try {
        const followId = `${userId}_${locationId}`;
        const followRef = doc(db, "follows_locations", followId);
 ...
 * @returns Success status and error message
 */
export async function updateLocationNotifications(locationId, userId, notifications) {
    try {
        const followId = `${userId}_${locationId}`;
        const followRef = doc(db, "follows_locations", followId);
 ...
 */
export async function createLocationPost(locationId, postData) {
    try {
        const postRef = await addDoc(collection(db, "locations", locationId, "threads"), {
            ...postData,
            createdAt: serverTimestamp(),
            updatedAt: serverTimestamp()
        });
 ...
 * @returns Success status and error message
 */
export async function updateLocationPost(locationId, postId, updates) {
    try {
 ...
 * @returns Success status and error message
 */
export async function deleteLocationPost(locationId, postId) {
    try {
        const postRef = doc(db, "locations", locationId, "threads", postId);
