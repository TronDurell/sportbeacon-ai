...
export interface ComposerAssistOptions {
  tenantId: string;
  enabled?: boolean;
...
export interface WritingSuggestion {
  id: string;
  type: 'style' | 'grammar' | 'tone' | 'content';
  text: string;
  confidence: number;
  reason?: string;
}
...
export function useComposerAssist(options: ComposerAssistOptions) {
  const [suggestions, setSuggestions] = useState<WritingSuggestion[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [writingStyle, setWritingStyle] = useState<UserWritingStyle | null>(null);
  const memorySDK = useRef<MemorySDK | null>(null);
      ...
      if (styleMemories.length > 0) {
        ...
        styleMemories.forEach(memory => {
          if (memory.text.includes('tone:')) {
            ...
            if (toneMatch) {
              style.tone = toneMatch[1] as UserWritingStyle['tone'];
            }
          }
          if (memory.text.includes('length:')) {
            const lengthMatch = memory.text.match(/length:(\w+)/);
            if (lengthMatch) {
              style.length = lengthMatch[1] as UserWritingStyle['length'];
            }
          }
          if (memory.text.includes('complexity:')) {
            const complexityMatch = memory.text.match(/complexity:(\w+)/);
            if (complexityMatch) {
              style.complexity = complexityMatch[1] as UserWritingStyle['complexity'];
            }
          }
  ...
  const learnFromFeedback = useCallback(async (suggestionId: string, feedback: 'positive' | 'negative') => {
    if (!memorySDK.current) return;
  ...
  const updateWritingStyle = useCallback(async (style: Partial<UserWritingStyle>) => {
    if (!memorySDK.current) return;
