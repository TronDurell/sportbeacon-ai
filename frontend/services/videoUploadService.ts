...
class VideoUploadService {
    ...
    async uploadVideo(
        file: File,
        metadata: Omit<VideoMetadata, 'id' | 'status' | 'uploadDate' | 'duration' | 'size' | 'format'>,
    ...
    async requestAnalysis(videoId: string, types: string[]): Promise<{
        analysisId: string;
    ...
    async getAnalysisResults(videoId: string, analysisId: string): Promise<{
    ...
    async generateThumbnail(videoId: string, timestamp: number): Promise<{
        thumbnailUrl: string;
    }> {
