...
class BadgeService {
    ...
    async checkBadgeProgress(profile) {
    ...
    calculateProgress(badge, profile) {
    ...
    async updateBadges(playerId, badges) {
        await fetchWithAuth(`/players/${playerId}/badges`, {
            method: 'PUT',
            body: JSON.stringify({ badges })
        });
    }
    async playUnlockSound(badge) {
        if (!badge.sound || !this.audioContext)
            return;
        try {
