...
export const VideoOverlay: React.FC<VideoOverlayProps> = ({
  ...
  const handleSetClipBoundary = (type: 'start' | 'end') => {
    ...
    setSelectedClip(prev => ({
      startTime:
        type === 'start' ? currentTime : prev?.startTime || currentTime,
  ...
  return (
    <div className="w-full" role="region" aria-label="Video overlay" tabIndex={0}>
      <Box
        sx={{
          position: 'relative',
          width: '100%',
            ...
            <Stack spacing={2}>
              ...
              {selectedClip && (
                <Typography variant="body2" color="text.secondary">
                  Duration:{' '}
                  {formatTimestamp(selectedClip.endTime - selectedClip.startTime)}
                </Typography>
              )}
            </Stack>
          </DialogContent>
