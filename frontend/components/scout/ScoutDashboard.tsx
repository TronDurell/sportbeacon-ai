...
export const ScoutDashboard: React.FC<ScoutDashboardProps> = ({
  ...
  const [selectedPlayer, setSelectedPlayer] = useState<ScoutPlayer | null>(
    null
  );
  ...
  const [comparisonType, setComparisonType] = useState<
    'team' | 'league' | 'none'
  >('none');
  const [comparisonStats, setComparisonStats] = useState<Record<
    string,
    number
  > | null>(null);
  ...
  const [filterImprovementArea, setFilterImprovementArea] =
    useState<string>('');
    ...
    const playerData = {
      player: {
        ...
        age:
          ...
          new Date(selectedPlayer.dateOfBirth).getFullYear(),
      ...
      videoClips: selectedPlayer.mediaUrls.highlightVideos.map(
        (url, index) => ({
          id: `clip-${index}`,
          url,
          timestamp: Date.now(),
  ...
  const fetchComparisonStats = useCallback(
    async (type: 'team' | 'league') => {
      try {
        const response = await fetch(
          `/api/stats/${type}-averages?teamId=${selectedPlayer?.currentTeam.id}`
        );
  ...
  const handleAddVideoNote = useCallback(
    (note: Omit<VideoNote, 'id' | 'createdAt'>) => {
      setVideoNotes(prev => [
        ...prev,
        {
          ...note,
  ...
  const fetchTips = async () => {
    try {
      ...
      const tipDocs = await getCollection<TipRecord>(q);
      setTips(tipDocs);
      setLastDoc(tipDocs[tipDocs.length - 1]);

      // Get total count
      const countSnapshot = await getCollection<TipRecord>(
        query(tipsRef, where('profileId', '==', scoutId))
      );
      setTotalTips(countSnapshot.length);
    } catch (error) {
  ...
  const fetchLeaderboardData = async () => {
    ...
    try {
      ...
      const leaderboardData = await getCollection<LeaderboardEntry>(q);
  ...
  const getSortField = (tab: TabValue): string => {
    switch (tab) {
      case 'weekly':
        return 'stats.weeklyTips';
      case 'views':
        return 'stats.viewCount';
      case 'shares':
        return 'stats.shareCount';
      case 'improved':
        return 'stats.streakScore';
      case 'badges':
        return 'stats.badge.percentile';
      default:
        return 'stats.weeklyTips';
    }
  };
  ...
  return (
    <Container maxWidth="xl" role="main" aria-label="Scout dashboard" tabIndex={0} sx={{ outline: 'none', ':focus': { boxShadow: 3 } }}>
...
export const CreatorInvoiceTable: React.FC<{ profileId: string }> = ({
  profileId,
}) => {
  const [tips, setTips] = useState<TipRecord[]>([]);
  const [loading, setLoading] = useState(true);
    ...
    <Paper>
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {tips.map(tip => (
              <TableRow data-testid="invoice-row" key={tip.id}>
                <TableCell data-testid="amount">
                  ${tip.amount.toFixed(2)}
                </TableCell>
                <TableCell data-testid="method">{tip.method}</TableCell>
...
export const getPayoutStatus = (
  ...
  if (
    !profile.payoutSettings?.stripeConnected &&
    !profile.payoutSettings?.cryptoWalletVerified
  ) {
...
}> = ({ open, onClose, recipientProfile, currentUserId }) => {
...
export const useEarnings = (profileId: string) => {
  return useQuery(
    ...
    async (): Promise<EarningsData> => {
      ...
      const [tipHistory, todayTips] = await Promise.all([
        getCollection<TipRecord>(tipsQuery),
        getCollection<TipRecord>(todayTipsQuery),
      ]);

      const totalEarnings = tipHistory.reduce(
        (sum, tip) => sum + tip.amount * tip.multiplier,
        0
      );
      ...
      const tipsToday = todayTips.length;
...
export const useTriggerTip = () => {
  ...
  return useMutation(
    async ({
      ...
      // Check daily limit
      const { dailyLimit } = await queryClient.fetchQuery([
        'earnings',
        profileId,
      ]);
      if (dailyLimit.remaining <= 0) {
        throw new Error(
          `Daily tip limit reached. Resets at ${dailyLimit.resetsAt.toLocaleTimeString()}`
        );
      }
...
export const CreatorAnalytics: React.FC<{ profileId: string }> = ({
  profileId,
}) => {
  const { data: analyticsData, isLoading: isAnalyticsLoading } =
    useCreatorAnalytics(profileId);
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: any) => {
            const dataPoint =
              analyticsData.tipVolumeOverTime[context.dataIndex];
            return [
