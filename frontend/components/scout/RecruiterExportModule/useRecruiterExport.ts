...
export const useRecruiterExport = () => {
  ...
  const fetchPlayerData = useCallback(
    async (playerId: string) => {
      try {
        ...
        const [player, aiAnalysis, videoClips, drillHistory] =
          await Promise.all([
            fetch(`/api/players/${playerId}`).then(res => res.json()),
            fetch(`/api/players/${playerId}/ai-analysis`).then(res =>
              res.json()
            ),
            fetch(`/api/players/${playerId}/video-clips`).then(res =>
              res.json()
            ),
            fetch(`/api/players/${playerId}/drill-history`).then(res =>
              res.json()
            ),
          ]);
  ...
  const shareViaEmail = useCallback(
    async (email: string, message: string, options: ExportOptions = {}) => {
      try {
        const blob = await generateExport(options);
