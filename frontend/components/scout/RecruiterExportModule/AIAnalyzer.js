const analyzeStats = (stats, position) => {
    const strengths = [];
    // Analyze based on position and stats
    if (position.toLowerCase().includes('forward')) {
        if (stats.goalsScored > 10)
            strengths.push('Consistent goal scorer');
        if (stats.shotAccuracy > 65)
            strengths.push('Excellent shot accuracy');
        if (stats.assists > 5)
            strengths.push('Good playmaking ability');
    }
    else if (position.toLowerCase().includes('midfielder')) {
        if (stats.passAccuracy > 80)
            strengths.push('Strong passing accuracy');
        if (stats.distanceCovered > 200)
            strengths.push('High work rate and stamina');
        if (stats.assists > 7)
            strengths.push('Creative playmaker');
    }
    else if (position.toLowerCase().includes('defender')) {
        if (stats.tacklesWon > 40)
            strengths.push('Strong tackling ability');
        if (stats.passAccuracy > 75)
            strengths.push('Good distribution from the back');
        if (stats.yellowCards < 5)
            strengths.push('Disciplined defender');
    }
    return strengths;
};
const generateRoleRecommendations = (stats, position) => {
    const roles = [];
...
const identifyImprovements = (stats, position) => {
    const improvements = [];
...
const generateRecommendations = (improvements) => {
