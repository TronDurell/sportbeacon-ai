...
export const generatePDF = async (
  ...
  if (options.customHeader) {
    doc.setFontSize(18);
    doc.text(options.customHeader, PAGE_WIDTH / 2, MARGIN + 10, {
      align: 'center',
    });
  }
    ...
    if (options.includeAIAnalysis && aiAnalysis) {
      ...
      aiAnalysis.strengths.forEach((strength: string, idx: number) => {
        doc.text(
          `• ${strength}`,
          MARGIN + 5,
          doc.lastAutoTable.finalY + 40 + idx * 7
        );
      });

      // Roles & Playing Style
      const rolesY =
        doc.lastAutoTable.finalY + 50 + aiAnalysis.strengths.length * 7;
