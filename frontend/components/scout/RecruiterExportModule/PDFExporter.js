...
const MARGIN = 20;
const addVideoClipsSection = (doc, clips) => {
    doc.setFontSize(14);
...
const addBadgesSection = (doc, badges) => {
    doc.setFontSize(14);
...
export const generatePDF = async (players, options = {}) => {
    const doc = new jsPDF();
    // Add header
    ...
    players.forEach((playerData, index) => {
        ...
        if (drillHistory.length > 0) {
            ...
            const drillData = drillHistory.map((drill) => [
                drill.name,
                drill.date,
        ...
        if (options.includeAIAnalysis && aiAnalysis) {
            ...
            aiAnalysis.roles?.forEach((role, idx) => {
            ...
            aiAnalysis.recommendations.forEach((recommendation, idx) => {
