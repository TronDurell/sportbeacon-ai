...
export const useRecruiterExport = () => {
    const { selectedPlayers, playerData, exportProgress, setExportProgress, updatePlayerData, } = useExport();
    ...
    const generateExport = useCallback(async (options = {}) => {
        try {
            setExportProgress({
                status: 'loading',
                progress: 0,
    ...
    const downloadExport = useCallback(async (options = {}) => {
        ...
        link.href = url;
        link.download = `scout_report_${new Date().toISOString().split('T')[0]}.${options.includeVideos ? 'zip' : 'pdf'}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
    }, [generateExport]);
    const shareViaEmail = useCallback(async (email, message, options = {}) => {
        try {
            const blob = await generateExport(options);
