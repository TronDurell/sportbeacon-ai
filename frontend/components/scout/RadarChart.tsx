...
const RadarChart: React.FC<RadarChartProps> = ({
  ...
  const formatData = () => {
    return STAT_CATEGORIES.map(category => {
      const categoryData = {
        ...
        player:
          category.stats.reduce(
            (acc, stat) => acc + (playerPercentiles[stat] || 0),
            0
      ...
      if (comparisonStats && comparisonType !== 'none') {
        categoryData[comparisonType] =
          category.stats.reduce(
            (acc, stat) => acc + (comparisonStats[stat] || 0),
            0
          ) / category.stats.length;
      }
  ...
  return (
    <Box role="region" aria-label="Radar chart" tabIndex={0} sx={{ outline: 'none', ':focus': { boxShadow: 3 }, mb: 2 }}>
      <Typography variant="h6" sx={{ mb: 1 }}>Player Stats Radar</Typography>
      <Box
        sx={{
          display: 'flex',
      ...
      >
        <ToggleButtonGroup
          ...
          size="small"
        >
          <ToggleButton value="none">Player Only</ToggleButton>
          <ToggleButton value="team">vs Team</ToggleButton>
          <ToggleButton value="league">vs League</ToggleButton>
        </ToggleButtonGroup>
      </Box>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <RechartsRadar data={formatData()}>
          ...
          {comparisonType !== 'none' && comparisonStats && (
            <Radar
              name={
                comparisonType === 'team' ? 'Team Average' : 'League Average'
              }
