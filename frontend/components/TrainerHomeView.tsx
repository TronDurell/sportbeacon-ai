...
export const TrainerHomeView: React.FC = () => {
  ...
  const { data: drillHistory, isLoading: historyLoading } = useQuery({
    ...
    queryFn: () =>
      selectedPlayer
        ? trainerAPI.getPlayerDrillHistory(selectedPlayer.id)
        : Promise.resolve([]),
  ...
  const feedbackStats = drillHistory?.reduce(
    (stats, drill) => {
      if (drill.feedback) {
        stats.totalFeedback++;
  ...
  const handleAssignDrill = useCallback(
    async (drill: DrillDetail) => {
  ...
  return (
    <div className="w-full" role="region" aria-label="Trainer home view" tabIndex={0}>
      {isMobile && (
