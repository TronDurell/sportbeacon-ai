...
export const PlayerDashboard: React.FC<PlayerDashboardProps> = ({
  ...
  const checkLevelAndBadges = async () => {
    ...
    const newlyEarnedBadge = badges.find(
      b => b.earned && !profile.badges?.some(pb => pb.id === b.id && pb.earned)
    );
    ...
    >
      <Box sx={{ width: 250, p: 2 }}>
        <Box display="flex" alignItems="center" mb={2}>
          <Avatar src={profile?.avatar} sx={{ width: 48, height: 48, mr: 1 }} />
          <Box>
  ...
  return (
    <div className="w-full" role="region" aria-label="Player dashboard" tabIndex={0}>
      <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
        {isMobile && (
          <Box sx={{ mb: 2 }}>
            <Fab size="small" color="primary" onClick={() => setDrawerOpen(true)}>
              <MenuIcon />
            </Fab>
          </Box>
        )}
            ...
            <Card sx={{ p: 2 }}>
              ...
              <List>
                ...
                {insights?.map(insight => (
                  <ListItem key={insight.date} divider>
                    <ListItemAvatar>
