import {onSiblingRequestCreated} from "../onSiblingRequestCreated";
import {
...
describe("onSiblingRequestCreated", () => {
  ...
  it("should process a new sibling request and create audit log", async () => {
    ...
    // Act
    await onSiblingRequestCreated(event as any);

    // Wait for async operations to complete
    await waitForAsync(200);

    // Assert
    const auditLogExists = await verifyAuditLogEntry("sibling_request_created", requestId, {
      townId,
  ...
  it("should handle sibling request with missing data gracefully", async () => {
    ...
    // Act & Assert
    await expect(onSiblingRequestCreated(event as any)).resolves.not.toThrow();
  });
  ...
  it("should handle sibling request with age group mismatch", async () => {
    ...
    const event = createMockDocumentCreatedEvent("towns/{townId}/siblingRequests/{requestId}", requestId, mismatchData);
  ...
  it("should handle sibling request with team capacity constraints", async () => {
    ...
    const event = createMockDocumentCreatedEvent("towns/{townId}/siblingRequests/{requestId}", requestId, capacityData);
  ...
  it("should handle sibling request with schedule conflicts", async () => {
    ...
    const event = createMockDocumentCreatedEvent("towns/{townId}/siblingRequests/{requestId}", requestId, scheduleData);
  ...
  it("should handle multiple sibling requests concurrently", async () => {
    ...
    for (const req of requests) {
      const auditLogExists = await verifyAuditLogEntry("sibling_request_created", req.requestId);
      expect(auditLogExists).toBe(true);
    }
  });
  ...
  it("should handle sibling request with special characters and complex data", async () => {
    ...
    const event = createMockDocumentCreatedEvent("towns/{townId}/siblingRequests/{requestId}", requestId, specialData);
  ...
  it("should handle sibling request with waitlist position differences", async () => {
    ...
    const event = createMockDocumentCreatedEvent("towns/{townId}/siblingRequests/{requestId}", requestId, waitlistData);
