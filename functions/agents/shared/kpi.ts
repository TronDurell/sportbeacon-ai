 ...
 */
export async function calculatePlayerStatKPIs(
  playerId: string,
  statType: string,
...
export async function calculateTeamKPIs(
  ...
  topPerformers: Record<string, { playerId: string; value: number }>;
  calculatedAt: Date;
} | null> {
  try {
    ...
    const topPerformers: Record<string, { playerId: string; value: number }> = {};
    Object.entries(teamStats).forEach(([statType, values]) => {
      ...
      const topPlayer = Object.entries(playerStats).find(([, stats]) => 
        stats[statType] === bestValue
      );
 ...
 */
export async function updatePlayerKPIs(playerId: string, statType: string, kpis: any): Promise<void> {
  try {
    const kpiDoc = {
      playerId,
      statType,
 ...
 */
export async function updateTeamKPIs(teamId: string, kpis: any): Promise<void> {
  try {
    const kpiDoc = {
      teamId,
      kpis,
 ...
 */
async function cacheKPICalculation(result: KPICalculationResult): Promise<void> {
  try {
 ...
 * Cache team KPIs
 */
async function cacheTeamKPIs(result: any): Promise<void> {
  try {
 ...
 */
async function updatePlayerKPISummary(playerId: string): Promise<void> {
  try {
 ...
 */
export async function getCachedKPICalculation(
  playerId: string,
  statType: string,
  timeRange: { from: Date; to: Date }
): Promise<KPICalculationResult | null> {
