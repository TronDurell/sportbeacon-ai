...
import { calculateTeamKPIs, updateTeamKPIs } from './shared/kpi.js';
import { storeAgentMemory, logAgentActivity } from './shared/memory.js';
 ...
 */
export const generateTeamReport = onCall(async (request) => {
  try {
 ...
 */
async function generateTeamWeeklyReport(
  teamId: string,
  teamData: any,
  customWeekRange?: { from: Date; to: Date }
  ...
  try {
    ...
    const teamKPIs = await calculateTeamKPIs(teamId, weekRange);
 ...
 */
async function processPendingSubmissions(teamId: string, weekRange: { from: Date; to: Date }): Promise<void> {
  try {
    ...
    for (const submissionDoc of pendingSubmissions.docs) {
      ...
      await db.collection('stats_submissions').doc(submissionDoc.id).update({
        status: 'verified',
 ...
 */
async function getPlayerPerformanceData(
  teamId: string,
 ...
 */
function generateRecommendations(teamKPIs: any, playerData: any[]): string[] {
...
async function storeReport(
  ...
  try {
    ...
    const reportRef = await db.collection('team_reports').add(reportData);
 ...
 */
async function sendReportNotifications(teamId: string, teamData: any, reportId: string): Promise<void> {
  try {
    // Notify coaches
 ...
 */
async function updateReportMemory(
  teamId: string,
  reportId: string,
  teamKPIs: any,
  weekRange: { from: Date; to: Date }
