 ...
 */
exports.createPlayerProfile = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
        // Validate input using Zod schema
        const createPlayerSchema = zod_1.z.object({
            playerData: validation_1.Schemas.CreatePlayer
        });
        const validation = validation_1.ValidationMiddleware.validateResponse(createPlayerSchema, data);
        ...
        try {
            await memoryClient.captureFunctionResult(auth.uid, 'createPlayerProfile', {
                playerId: playerProfile.id,
                playerName: (playerProfile.firstName || '') + ' ' + (playerProfile.lastName || ''),
                dateOfBirth: playerProfile.dateOfBirth,
                position: playerProfile.position
            }, undefined, 'player-profile-creation');
        }
        catch (memoryError) {
        ...
        try {
            ...
            await memoryClient.captureFunctionError(authContext.auth?.uid || 'unknown', 'createPlayerProfile', err, { input: data }, 'player-profile-creation-error');
        }
        catch (memoryError) {
 ...
 */
exports.updatePlayerProfile = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
        // Validate input using Zod schema
        const updatePlayerSchema = zod_1.z.object({
            playerId: zod_1.z.string().uuid("Invalid player ID format"),
            updates: validation_1.Schemas.UpdatePlayer
        });
        const validation = validation_1.ValidationMiddleware.validateResponse(updatePlayerSchema, data);
        if (!validation.success) {
            return {
                success: false,
 ...
 */
exports.getPlayerStatistics = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
 ...
 */
exports.getPlayerAchievements = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
 ...
 */
exports.awardAchievement = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
        ...
        await db.collection("players").doc(playerId).update({
            "stats.achievements": firestore_1.FieldValue.arrayUnion(achievementDoc.type),
            "updatedAt": new Date(),
        });
        // Create audit log
        await db.collection("townStaffAuditLogs").add({
 ...
 */
exports.getPlayerSchedule = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
        const { playerId } = data.data;
 ...
 */
exports.updatePlayerPerformance = (0, https_1.onCall)(async (data, context) => {
    try {
        const auth = await validateAuth(context);
