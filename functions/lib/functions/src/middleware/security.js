...
exports.SECURITY_EVENTS = exports.RATE_LIMITS = exports.detectSuspiciousActivity = exports.errorHandler = exports.auditLog = exports.requireRole = exports.requirePermission = exports.validateInput = exports.rateLimit = void 0;
...
exports.SECURITY_EVENTS = SECURITY_EVENTS;
/**
 * Rate limiting middleware
 */
const rateLimit = (operation, limit, window) => {
    return async (data, context) => {
        ...
        // Check rate limit
        const rateLimitDoc = await index_1.db.collection("rateLimits").doc(userId).get();
        ...
        if (lastReset < windowStart) {
            await index_1.db.collection("rateLimits").doc(userId).update({
                [key]: 1,
                [`${key}_reset`]: now
            });
        }
            ...
            throw new functions.https.HttpsError("resource-exhausted", `Rate limit exceeded for ${operation}. Limit: ${limit} requests per ${window}`);
        }
        else {
            // Increment counter
            await index_1.db.collection("rateLimits").doc(userId).update({
...
const validateInput = (schema) => {
    return (data, context) => {
        ...
        if (errors.length > 0) {
            ...
            throw new functions.https.HttpsError("invalid-argument", `Input validation failed: ${errors.join(", ")}`);
        }
    };
};
...
const requirePermission = (permission) => {
    return (data, context) => {
        ...
        if (!hasPermission) {
            ...
            throw new functions.https.HttpsError("permission-denied", `Insufficient permissions. Required: ${permission}`);
        }
    };
};
...
const requireRole = (role) => {
    return (data, context) => {
        ...
        if (!hasRole) {
            ...
            throw new functions.https.HttpsError("permission-denied", `Insufficient role. Required: ${role}`);
        }
    };
};
 ...
 */
const detectSuspiciousActivity = (data, context) => {
    const userId = context.auth?.uid;
    if (!userId)
        return;
...
// Helper functions
function getWindowStart(now, window) {
...
function checkUserPermission(token, permission) {
    const permissions = {
    ...
    if (token.admin)
        return true;
    if (token.moderator && permissions.moderator.includes(permission))
        return true;
    if (token.creator && permissions.creator.includes(permission))
        return true;
    if (permissions.user.includes(permission))
        return true;
    return false;
}
async function checkRapidRequests(userId) {
    const now = admin.firestore.Timestamp.now();
    const oneMinuteAgo = admin.firestore.Timestamp.fromDate(new Date(now.toDate().getTime() - 60 * 1000));
    const recentRequests = await index_1.db.collection("securityLogs")
        .where("userId", "==", userId)
...
function checkUnusualPayment(data) {
...
async function checkFailedAttempts(userId) {
    const now = admin.firestore.Timestamp.now();
    const oneHourAgo = admin.firestore.Timestamp.fromDate(new Date(now.toDate().getTime() - 60 * 60 * 1000));
    const failedAttempts = await index_1.db.collection("securityLogs")
        .where("userId", "==", userId)
...
function checkGeographicAnomaly(context) {
...
async function logSecurityEvent(event) {
    try {
        await index_1.db.collection("securityLogs").add({
            ...event,
            timestamp: admin.firestore.Timestamp.now(),
            ipAddress: "unknown",
