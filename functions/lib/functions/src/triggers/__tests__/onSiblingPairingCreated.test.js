...
describe("onSiblingPairingCreated", () => {
    ...
    it("should process a new sibling pairing request and create audit log", async () => {
        ...
        const pairingData = test_utils_1.mockData.siblingPairing;
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, pairingData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
        // Assert
        const auditLogExists = await (0, test_utils_1.verifyAuditLogEntry)("sibling_pairing_created", pairingId, {
            status: "processing",
        });
    ...
    it("should handle sibling pairing with missing data gracefully", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, {});
        // Act & Assert
        await expect((0, index_1.onSiblingPairingCreated)(event)).resolves.not.toThrow();
    });
    it("should process sibling pairing with complete data", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, completePairingData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
    ...
    it("should handle sibling pairing with multiple children", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, multipleChildrenData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
    ...
    it("should handle sibling pairing with different leagues", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, differentLeaguesData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
    ...
    it("should handle sibling pairing with age conflicts", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, ageConflictData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
    ...
    it("should handle multiple sibling pairing requests concurrently", async () => {
        // Arrange
        const pairings = [
            { id: "pairing-1", data: { ...test_utils_1.mockData.siblingPairing, familyId: "family1", children: [{ name: "Child 1A", age: 8 }, { name: "Child 1B", age: 6 }] } },
            { id: "pairing-2", data: { ...test_utils_1.mockData.siblingPairing, familyId: "family2", children: [{ name: "Child 2A", age: 9 }, { name: "Child 2B", age: 7 }] } },
            { id: "pairing-3", data: { ...test_utils_1.mockData.siblingPairing, familyId: "family3", children: [{ name: "Child 3A", age: 10 }, { name: "Child 3B", age: 8 }] } }
        ];
        // Act
        const promises = pairings.map((pairing) => {
            const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairing.id, pairing.data);
            return (0, index_1.onSiblingPairingCreated)(event);
        });
        ...
        for (const pairing of pairings) {
            const auditLogExists = await (0, test_utils_1.verifyAuditLogEntry)("sibling_pairing_created", pairing.id);
            expect(auditLogExists).toBe(true);
        }
    });
    it("should handle sibling pairing with special characters and complex data", async () => {
        ...
        const event = (0, test_utils_1.createMockDocumentCreatedEvent)("siblingPairings/{pairingId}", pairingId, specialData);
        // Act
        await (0, index_1.onSiblingPairingCreated)(event);
        // Wait for async operations to complete
        await (0, test_utils_1.waitForAsync)(200);
...
//# sourceMappingURL=onSiblingPairingCreated.test.js.map