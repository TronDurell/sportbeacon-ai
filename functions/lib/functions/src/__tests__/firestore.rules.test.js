...
describe("Firestore Rules - Location Threads (Comprehensive)", () => {
    ...
    beforeAll(async () => {
        testEnv = await (0, rules_unit_testing_1.initializeTestEnvironment)({
            ...
            firestore: {
                rules: (0, fs_1.readFileSync)((0, path_1.join)(__dirname, "../firestore.rules"), "utf8"),
            },
        });
    });
    afterAll(async () => {
    ...
    // Test utilities
    const createTestLocation = async (locationId, moderators = []) => {
        const locationData = {
            name: "Test Location",
            slug: "test-location",
            sport: "basketball",
    ...
    const createTestPost = async (locationId, postId, authorId) => {
        const postData = {
            locationId,
            authorId,
            type: "note",
    ...
    describe("Follows Collection", () => {
        ...
        it("should validate notification preference values", async () => {
            ...
            const invalidFollowData = {
                locationId,
                userId,
                notifications: "invalid-preference",
                createdAt: new Date()
            };
            await expect(followDoc.set(invalidFollowData)).rejects.toThrow();
        });
